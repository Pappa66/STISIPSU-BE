        ...
        const items = await prisma.repositoryItem.findMany({ 
            ...
            select: {
                ...
                files: {
                    ...
                    take: 1 // Hanya ambil satu file pertama untuk link unduh
                }
            }
        });
...
const getAllRepositoryItemsForAdmin = async (req, res, next) => {
    try {
        ...
        const totalItems = await prisma.repositoryItem.count({ where: whereClause });
...
const getRepositoryItemById = async (req, res, next) => {
    ...
    try {
        ...
        const isPubliclyVisible = item.visibility === 'PUBLISHED' && item.approvalStatus === 'APPROVED';
        
        // Cek otorisasi untuk item yang tidak publik
        if (!isPubliclyVisible && (!req.user || req.user.role !== 'ADMIN')) {
             return res.status(403).json({ message: 'Anda tidak memiliki akses untuk melihat item ini.' });
        }
...
const updateRepositoryItem = async (req, res, next) => {
    const { id } = req.params;
    const { title, author, year, studyProgram, abstract, keywords, advisorId, visibility, approvalStatus, rejectionReason, showDownloadsToPublic } = req.body;
    
    try {
        const dataToUpdate = {
            title, author, studyProgram, abstract, keywords, advisorId,
            visibility, approvalStatus, rejectionReason, showDownloadsToPublic,
...
const deleteRepositoryItem = async (req, res, next) => {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            const itemToDelete = await tx.repositoryItem.findUnique({ where: { id }, include: { files: true } });
            ...
            for (const file of itemToDelete.files) {
                if (!file.fileUrl.startsWith('http')) {
                    ...
                    try { await fs.unlink(filePath); } catch (unlinkError) { console.warn(`File tidak ditemukan saat mencoba menghapus: ${filePath}`); }
                }
            }
...
const deleteFileItem = async (req, res, next) => {
    ...
    try {
        const fileToDelete = await prisma.fileItem.findUnique({ where: { id: fileId } });
        if (!fileToDelete) return res.status(404).json({ message: 'File tidak ditemukan.' });
        if (!fileToDelete.fileUrl.startsWith('http')) {
            const filePath = path.join(process.cwd(), 'public', fileToDelete.fileUrl);
            try { await fs.unlink(filePath); } catch (unlinkError) { console.warn(`File di disk tidak ditemukan: ${filePath}`); }
        }
