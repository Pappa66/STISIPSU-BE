...
const updatePost = async (req, res, next) => {
    ...
    try {
        const updatedPost = await prisma.post.update({ 
            where: { id }, 
            data: { title, blocks, slug, featuredImageUrl } 
        });
        res.json(updatedPost);
    } catch (error) {
        if (error.code === 'P2025') return res.status(404).json({ message: 'Postingan tidak ditemukan.' });
        next(error);
    }
};
...
const deletePost = async (req, res, next) => {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            await tx.menuItem.updateMany({ where: { postId: id }, data: { postId: null } });
            await tx.subMenuItem.updateMany({ where: { postId: id }, data: { postId: null } });
            await tx.post.delete({ where: { id } });
        });
        res.json({ message: 'Post berhasil dihapus.' });
    } catch (error) {
        if (error.code === 'P2025') return res.status(404).json({ message: 'Post tidak ditemukan.' });
        next(error);
    }
};
